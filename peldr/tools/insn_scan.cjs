// Disasm an RVA window and print instructions matching a substring.
const fs = require("fs");
...
for (let i = 0; i < nsec; i++) {
  const p = so + i * 40;
  secs.push({ vs: B.readUInt32LE(p + 8), va: B.readUInt32LE(p + 12), rs: B.readUInt32LE(p + 16), pr: B.readUInt32LE(p + 20) });
}
...
while (pos < len) {
  ...
  try {
    for (const ins of md.disasm(chunk, { address: imageBase + lo + pos })) {
      ...
      if (text.includes(pat)) { console.log(`RVA ${(ins.address - imageBase).toString(16)}: ${text}`); count++; }
    }
  } catch (err) {}
  pos += (consumed || 1);
}
